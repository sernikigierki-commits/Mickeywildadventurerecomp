...
void memory_init(const char* bios_path) {
    ...
    /* PSX_EMBEDDED_OPENBIOS_DIRECT: no temporary file and no external BIOS.
     * The generated C++ translation unit owns the exact 512 KiB image. */
    if (bios_path && strcmp(bios_path, "__PSXRECOMP_EMBEDDED_OPENBIOS__") == 0) {
        extern int psx_embedded_openbios_copy(unsigned char* dst, unsigned int capacity);
        n = (size_t)psx_embedded_openbios_copy(bios_rom, BIOS_ROM_SIZE);
    } else
#endif
    {
        ...
        n = fread(bios_rom, 1, BIOS_ROM_SIZE, f);
        fclose(f);
    }
    if (n != BIOS_ROM_SIZE) {
