...
bool sha256_file(const std::filesystem::path& path, std::string& out,
    ...
    // Android's SDL_main thread can have a 1 MiB native stack. Keeping this
    // equally large hashing buffer as a local object crosses its guard page on
    // 32-bit ARM before the first statement in this function can run.
