...
#define LOAD(p, n) do { p = (void *)SDL_GL_GetProcAddress(n); if (!p) ok = 0; } while (0)
    ...
    if (!p_glGetQueryObjectui64v)
        p_glGetQueryObjectui64v = (void *)SDL_GL_GetProcAddress("glGetQueryObjectui64vEXT");
    if (!p_glQueryCounter)
        p_glQueryCounter = (void *)SDL_GL_GetProcAddress("glQueryCounterEXT");
#endif
#undef LOAD
    return ok;
}
...
static uint64_t  s_rt_up_diag[10];
...
void gl_renderer_runtime_diag(uint64_t out[10]) {
    for (int i = 0; i < 10; i++) out[i] = s_rt_up_diag[i];
}
...
static void hold_capture_drawable(void) {
#ifdef MICKEY_ANDROID_GAMEPLAY_PROFILE
    uint64_t capture_start = runtime_upload_diag_enabled()
    ...
    if (capture_start) {
        s_rt_up_diag[8]++;
        s_rt_up_diag[9] += SDL_GetPerformanceCounter() - capture_start;
    }
#endif
}
...
#if defined(__ANDROID__)
    "  vec2 uv = (v_persp != 0) ? v_uv_p : v_uv * gl_FragCoord.w;\n"
#else
...
#if defined(__ANDROID__)
    "  if (v_raw == 0) rgb = clamp(rgb * v_col.rgb * gl_FragCoord.w * 2.0, 0.0, 1.0);\n"
#else
...
static GLuint compile_shader(GLenum type, const char *src) {
    ...
    char *converted = NULL;
    if (strncmp(src, desktop, strlen(desktop)) == 0) {
        size_t n = strlen(es) + strlen(src + strlen(desktop)) + 1;
        converted = (char *)malloc(n);
        if (!converted) return 0;
        snprintf(converted, n, "%s%s", es, src + strlen(desktop));
        src = converted;
    }
#endif
...
static GLuint build_program_ex(const char *vs, const char *fs, int dual_source) {
    ...
    if (dual_source && p_glBindFragDataLocationIndexed) {
...
int gl_renderer_init_context(SDL_Window *win) {
    ...
    fprintf(stdout,
            "psxrecomp: OpenGL context created (vendor=%s, renderer=%s, version=%s, glsl=%s)\n",
            vendor ? vendor : "?", renderer ? renderer : "?",
            ver ? ver : "?", glsl ? glsl : "?");
#if defined(__ANDROID__)
    __android_log_print(ANDROID_LOG_INFO, "MickeyGL",
                        "vendor=%s renderer=%s version=%s glsl=%s",
                        vendor ? vendor : "?", renderer ? renderer : "?",
                        ver ? ver : "?", glsl ? glsl : "?");
#endif
...
void gl_renderer_present_blank(void) {
     ...
     * MICKEY / BEZEL FIX
     *
     * Gry PS1 czesto wylaczaja display na kilka klatek przy zmianie sceny.
     * Stary kod czyscil wtedy CALY host window do czerni, przez co bezel
     * znikal na jedna/dwie klatki i wygladal jak migotanie.
     *
     * Bezel jest host-side, wiec powinien pozostac nieruchomy.
     * Czyscimy tylko aperturę gry 4:3.
     */
    ...
    if (gl_renderer_has_bezel()) {
        /*
         * Mickey zostaje natywnie 4:3 niezaleznie od rozmiaru
         * hostowego okna.
         */
        letterbox_rect_aspect(
            ww, wh,
            4, 3,
            &lx, &ly, &lw, &lh
        );
        ...
        /*
         * Najpierw narysuj nieruchoma ramke na calym drawable.
         */
        present_bezel(
            ww, wh,
            lx, ly, lw, lh
        ...
        /*
         * Potem wyczysc TYLKO miejsce, w ktorym znajduje sie obraz PS1.
         * Boki z ramka zostaja nietkniete.
         */
        glEnable(GL_SCISSOR_TEST);
...
static void gl_swap_with_osd(void) {
    ...
    mickey_frontend_draw_game_transition();
#ifdef MICKEY_ANDROID_GAMEPLAY_PROFILE
    uint64_t swap_start = runtime_upload_diag_enabled()
        ? SDL_GetPerformanceCounter() : 0;
#endif
    ...
    if (swap_start) {
        s_rt_up_diag[6]++;
        s_rt_up_diag[7] += SDL_GetPerformanceCounter() - swap_start;
    }
#endif
}
